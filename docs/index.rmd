...
library(tidyverse)
library(here) #<1> directory stucture
library(gt) # <2>tables
library(janitor) # <3>cleaning data
library(ggplot2) # <4>plotting graph
library(sf) # <5>to read in map data 
library(readxl) #<6> to read in map data
library(plotly) # <7>to make interactive
library(viridis) # <8>colour palette for map
library(ggiraph)

```
1.'here' is a directory structure 2.'gt' is for making tables 3. 'janitor' is for cleaning data 4.'ggplo2' is for plotting graph 5. 'sf' is to read in map data 6. plotly is to make it interactive 7. 'virdis' is responsible for the colour pattern of the map 
```{r message=FALSE, warning=FALSE}
...
```
*results*
The line graph shows a consistent upward trend in antidepressant prescribing during the winter months across all years observed. Although a reference line marks the COVID-19 period (2020–2022), the overall trajectory does not display a sharp deviation from pre-pandemic patterns. The rate of increase appears slightly less steep during the COVID-19 years compared with earlier periods such as 2018–2019, suggesting that the pandemic may have had only a modest effect on winter prescribing behaviour. Nevertheless, the underlying upward trend persists throughout the entire time series. The y-axis does not begin at zero, allowing clearer visualisation of year-to-year differences.
...
covid line
map - freescale or change the gradient colours
round numbers
avg = median and range - use statistic 
box plot before map
literature - rich people take depressant (in introduction) - deprssion affects 
lolipop - quintiles instad decile ?- how to rank to depreva
changw colour map
limitation